...
int main(int argc, char** argv)
{
  ...
  //  parser_2D_images( argv[1], points2d );
    vector<string> images_paths;
    getdir( argv[1], images_paths );

    for (int i = 0; i < images_paths.size(); i++)
    {
        Mat image = imread(images_paths.at(i),CV_LOAD_IMAGE_COLOR);
        Ptr<ORB> detector = ORB::create();
        std::vector<KeyPoint> kp;
        detector->detect(image, kp);

        Mat_<double> frame(2, kp.size());
        for (int j = 0; j < kp.size(); ++j)
        {
            frame(0,j) = kp.at(j).pt.x;
            frame(1,j) = kp.at(j).pt.y;
            std::cout << "x:" << frame(0,j) << " / y:" << frame(1,j) << std::endl;
        }
        points2d.push_back(Mat(frame));
        ...
        drawKeypoints( image, kp, img_keypoints_1, Scalar::all(-1), DrawMatchesFlags::DEFAULT );
