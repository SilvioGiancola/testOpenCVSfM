...
int main(int argc, char* argv[])
{
  ...
  //  for (int i = 0; i < fileInfoList.size(); i++)
    ...
    {
        images_paths.push_back(fileInfoList.at(i).absoluteFilePath().toStdString());
    }
    // Build instrinsics
    float f = atof(argv[2]),
            cx = atof(argv[3]),
    ...
    std::cout << " -> image opened in " << t.elapsed() << std::endl;
    reconstruct(images_paths, Rs_est, ts_est, K, points3d_estimated, is_projective);
    std::cout << " -> reconstructed in " << t.elapsed() << std::endl;
    // Print output
    cout << "\n----------------------------\n" << endl;
    ...
    std::cout << " -> post in " << t.elapsed() << std::endl;
    window.spin();
    return 0;
}
